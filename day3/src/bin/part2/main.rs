...
struct Map{
    ...
    touch: HashMap<Symbol,Vec<Number>>,
}

impl Map{
    fn new() -> Map{
        Map{sum: 0, last_line_sym: vec!(), current_line_sym: vec!(), last_line_num: vec!(), current_line_num: vec!(), touch: HashMap::new()}
    }
    ...
    fn add_num(&mut self, val: Number){
        for sym in &self.last_line_sym{
            if touch(&val.pos, &sym.pos){
                ...
                self.touch.entry(sym.clone()).or_default().push(val);
                return;
            }
        }
    ...
    fn add(&mut self, sym: Symbol){
        for touched in self.last_line_num.extract_if(|x| touch(&x.pos, &sym.pos)){
            ...
            self.touch.entry(sym.clone()).or_default().push(touched);
        }
    ...
    fn print_remaining(&self){
        println!("\n\n----Gear numbers: ");
        let mut sum = 0;
        for val in &self.touch{
            println!("got {}", val.1.len());
            if val.1.len() == 2{
                println!("added {:?}", val.1);
                sum += val.1.get(0).unwrap().val * val.1.get(1).unwrap().val;
            }else if val.1.len() > 2 {
                panic!("what");
            }
        }
        println!("sum gear is {}", sum);
    }
}
